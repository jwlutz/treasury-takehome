...
// load .env (the lazy OpenAI client reads OPENAI_API_KEY at call time, so setting it here is enough)
function loadEnv() {
...
const appOf = (r: Row): ApplicationFields => ({ beverage_type: r.beverage_type, ...r.application_fields });
...
async function main() {
  ...
  const rows: Row[] = readFileSync(join(DIR, 'ground_truth.jsonl'), 'utf8').trim().split('\n').map((l) => JSON.parse(l));
  ...
  writeFileSync(join(DIR, 'results.json'), JSON.stringify({ mode: logicOnly ? 'logic' : 'model', n: results.length, accuracy: acc, logicAccuracy: logicAcc, reviewRate, falseClears: falseClears.map((x) => x.id), falseRejects: falseRejects.map((x) => x.id), avgLatencyMs: avgLatency, ranAtMs: Date.now(), totalMs: Date.now() - t0, results }, null, 2));
